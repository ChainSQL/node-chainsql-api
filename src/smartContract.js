...
function handleDeployTx(contractObj, signedVal, callback, resolve, reject){
	...
	// subscribe event
	chainSQL.event.subscribeTx(signedVal.id, /*isFunction ? callback :*/async function(err, data) {
		if (err) {
			errFunc(err);
		} else {
			// success
            ...
            // } else if (callback != undefined && callback.expect === data.status 
			// 			&& data.type === 'singleTransaction') {
            } else if (callback != undefined && data.status === 'validate_success' 
                && data.type === 'singleTransaction') {
                    let contractAddr = await getContractAddr(chainSQL, data.transaction.hash);
                    if(contractAddr === ""){
                        errFunc({
    ...
    if(args.generateRequest) {
        ...
        switch (args.type) {
            ...
            case 'call':
                ...
                break;
                //handleContractCall(this, args.options, args.callback);
