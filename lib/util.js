...
{
    // for secp256k1, the keylen is 32, cause ecies decode symkey as utf-8 rather than hex.
    return cryptoo.randomBytes(AESKeyLength/2).toString('hex');
}
...
function decodeToken(secret, token) {
    ...
    if (secret === "gmAlg") {
        symKey = keypairs.gmAlgSm2Dec(secret, token);
    } else if (regSoftGMSeed.test(secret)) {
        symKey = keypairs.softGMAlgSm2Dec(secret, token);
    } else {
