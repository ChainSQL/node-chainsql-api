...
function eciesEncrypt(message, publicKey) {
    ...
    else if(zArray.length < 32) {
        let padNum = 32 - zArray.length;
        for(let num = 0; num < padNum; num ++)
        {
            zArray.unshift(0);
        }
    }
    var kdfOutput = hashjs.sha512().update(zArray).digest();
 ...
 */
var symEncrypt = function(symKey, plaintext, algType = 'aes', plainTxtFormat = 'utf8') {
	if(algType === "gmAlg") {
		return keypairs.gmAlgSymEnc(symKey, plaintext);
    } else if(algType === "softGMAlg"){
	    return keypairs.softGMAlgSymEnc(symKey, plaintext);
	} else {
		return aesEncrypt(symKey, plaintext, plainTxtFormat);
	}
};

var aesEncrypt = function(secret, plaintext, plainTxtFormat = 'utf8') {
    ...
    var plainBuf = Buffer.from(plaintext, plainTxtFormat);
    var encryptedBytes = cipher.update(plainBuf);
    encryptedBytes = Buffer.concat([encryptedBytes, cipher.final()]);
 ...
 */
var symDecrypt = function(symKey, encryptedHex, algType = 'aes', plainTxtFormat = 'utf8') {
	if(algType === "gmAlg") {
		return keypairs.gmAlgSymDec(symKey, encryptedHex);
    } else if(algType === "softGMAlg"){
	    return keypairs.softGMAlgSymDec(symKey, encryptedHex);
	}else {
		return aesDecrypt(symKey, encryptedHex, plainTxtFormat);
	}
};

var aesDecrypt = function(secret, encryptedHex, plainTxtFormat = 'utf8') {
    var secretPadded = paddingPass(secret,AESKeyLength);
    var aesKey =Buffer.from(secretPadded, 'utf8');
    var iv = aesKey.slice(0, IVLength);
    ...
    return decryptedBytes.toString(plainTxtFormat);
};
